...
import { Home as HomeIcon, Bot, MessageCircle, BookOpen, LogOut, User, AlertCircle, ArrowLeft, ArrowRight } from 'lucide-react'
...
function Home() {
  ...
  return (
    <div className="flex min-h-screen">
      ...
      {/* Sidebar */}
      <div className="w-64 rounded-xl mt-3 shrink-0 p-4 flex flex-col gap-2 bg-white/10 backdrop-blur-md border-r border-white/20">

        <Link to="/" className="flex items-center gap-3 text-white hover:bg-white/20 px-3 py-2.5 rounded-xl transition-all">
        <HomeIcon size={18}/><span> Home</span>
        </Link>
        <Link to="/ai-chat" className="flex items-center gap-3 text-white hover:bg-white/20 px-3 py-2.5 rounded-xl transition-all">
        <Bot size={18}/><span>AI Chat</span>
        </Link>
        <Link to="/doctor-chat" className="flex items-center gap-3 text-white hover:bg-white/20 px-3 py-2.5 rounded-xl transition-all">
        <MessageCircle size={18}/><span>Doctor Chat</span>
        </Link>
        <Link to="/blog" className="flex items-center gap-3 text-white hover:bg-white/20 px-3 py-2.5 rounded-xl transition-all">
        ...
        <div className="mt-auto">
          <div className="bg-gradient-to-br from-violet-600/30 to-purple-600/30 border border-white/20 rounded-2xl p-4">
            ...
            <Link to="/ai-chat" className="block text-center bg-gradient-to-r from-violet-600 to-purple-600 text-white text-sm font-semibold py-2 rounded-xl">
      ...
      <div className="flex-1 overflow-y-auto p-3">
        ...
        >
          {features.map((feature, index) => (
            <SwiperSlide key={index}>
              <div className={`relative bg-gradient-to-br ${feature.bg} text-white rounded-3xl overflow-hidden h-72`}>
                ...
                <div className="relative z-10 flex flex-col justify-center h-full px-12">
                  ...
                  <h2 className="text-4xl font-bold mb-3">{feature.title}</h2>
                  <p className="text-lg opacity-90 max-w-lg">{feature.description}</p>
                </div>
        ...
        <div className="flex justify-between items-center mb-4">
          ...
          <Link to="/blog" className="text-violet-400 hover:text-violet-300 text-sm"><span className='flex'> সব দেখুন <ArrowRight size={18}></ArrowRight></span></Link>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {posts.slice(0, 6).map((post) => (
              <div key={post.id} className="bg-white/10 backdrop-blur-md border border-white/20 rounded-3xl p-5 hover:bg-white/15 transition-all shadow-xl">
                ...
                <span>
                  <User />  <span className="text-white/40 text-xs">{post.authorName}</span>
                </span>
